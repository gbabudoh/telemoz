...
import { getGoogleTokens } from "@/lib/integrations/google";
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(new URL("/pro/settings?error=google_auth_failed", req.url));
  }
  ...
  try {
    ...
    await prisma.integrationCredential.upsert({
      ...
      update: {
        ...
        refreshToken: tokens.refresh_token ? encrypt(tokens.refresh_token as string) : undefined,
    ...
    return NextResponse.redirect(new URL("/pro/settings?success=google_connected", req.url));
  } catch (error) {
