...
import {
  ...
  Shield, Edit2, Crown, UserCheck, Clock, Mail, ChevronRight,
} from "lucide-react";
...
export default function TeamPage() {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setFormError(data.error ?? "Failed to send invite"); return; }
  ...
  const handleRevokeInvite = async (id: string) => {
    setActionError("");
    setRevoking(id);
    const res = await fetch(`/api/pro/team/invites?id=${id}`, { method: "DELETE" });
    if (res.ok) {
      setPendingInvites(prev => prev.filter(i => i.id !== id));
    } else {
      setActionError("Failed to revoke invite.");
    }
    setRevoking(null);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => seatLimit > 0 ? setIsInviting(true) : setActionError("Upgrade your plan to add team members.")}
          className="h-11 px-5 rounded-xl bg-gray-900 hover:bg-black text-white font-bold text-sm flex items-center gap-2 cursor-pointer shadow-sm disabled:opacity-50"
        >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="font-black text-gray-900 text-sm">Seat Usage</p>
            <p className="text-xs text-gray-400 font-medium mt-0.5 capitalize">
              {tier} plan · {seatCount} of {seatLimit} seats used
              {tier !== "free" && <span className="ml-2 text-[#0a9396]">· Billed per seat/month</span>}
            </p>
          </div>
          <div className="text-right">
            <span className="text-2xl font-black text-gray-900">{seatCount}</span>
            <span className="text-sm text-gray-400 font-bold">/{seatLimit}</span>
          </div>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-[#0a9396] to-emerald-400 rounded-full transition-all duration-500"
            style={{ width: seatLimit > 0 ? `${Math.min(100, (seatCount / seatLimit) * 100)}%` : "0%" }}
          />
        </div>
        {tier === "free" && (
          <div className="mt-3 flex items-center justify-between">
            <p className="text-xs text-amber-600 font-semibold">Team seats require a paid plan</p>
            <button
              onClick={() => window.location.href = "/pro/settings?activeTab=billing"}
              className="text-xs font-black text-[#0a9396] hover:underline flex items-center gap-1"
            >
              Upgrade <ChevronRight className="h-3 w-3" />
            </button>
          </div>
        )}
        {tier !== "free" && seatsRemaining <= 2 && seatLimit > 0 && (
          <p className="text-xs text-amber-600 font-semibold mt-3">
            Only {seatsRemaining} seat{seatsRemaining !== 1 ? "s" : ""} remaining
          </p>
        )}
      </div>
        ...
        <>
          ...
          {members.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-xs font-black uppercase tracking-widest text-gray-400">Active Members</h2>
              {members.map(m => {
                ...
                return (
                  <div key={m.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-5 hover:border-gray-200 transition-all">
                    {m.member.image ? (
                      ...
                      <img src={m.member.image} alt={m.member.name} className="h-12 w-12 rounded-2xl object-cover shrink-0" />
                    ) : (
          ...
          {pendingInvites.length > 0 && (
            <div className="space-y-3">
              ...
              {pendingInvites.map(inv => (
                <div key={inv.id} className="bg-white rounded-2xl border border-dashed border-gray-200 p-5 flex items-center gap-4">
                  <div className="h-12 w-12 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center shrink-0">
                    <Mail className="h-5 w-5 text-gray-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-black text-gray-900">{inv.email}</h3>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${roleConfig[inv.role]?.color ?? "bg-gray-100 text-gray-500"}`}>
                        {roleConfig[inv.role]?.icon}
                  ...
                  <button
                    ...
                    className="h-9 w-9 rounded-xl border border-gray-200 flex items-center justify-center text-red-400 hover:text-red-600 cursor-pointer disabled:opacity-50 shrink-0"
                    title="Revoke invite"
                  >
                    <X className="h-3.5 w-3.5" />
                  </button>
          ...
          {members.length === 0 && pendingInvites.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              {seatLimit > 0 ? (
                ...
                <button onClick={() => window.location.href = "/pro/settings?activeTab=billing"}
                  className="h-10 px-5 rounded-xl bg-[#0a9396] text-white font-bold text-sm flex items-center gap-2 cursor-pointer">
          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm"
            ...
            <motion.div className="bg-white rounded-3xl shadow-2xl w-full max-w-md"
              ...
              <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-gray-900">Invite Team Member</h2>
                  <p className="text-xs text-gray-400 font-medium mt-0.5">
                    {seatsRemaining} seat{seatsRemaining !== 1 ? "s" : ""} remaining · Billed per seat/month
                  </p>
                </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-gray-400">An invite email will be sent. They can sign up if they don't have an account.</p>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={handleInvite} disabled={saving}
                    ...
                    <Mail className="h-4 w-4" />{saving ? "Sending…" : "Send Invite"}
                  </button>
                </div>
              </div>
            </motion.div>
